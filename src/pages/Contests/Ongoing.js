...
const Ongoing = ({ darkmode }) => {
    ...
    const fetchAllContests = async () => {
        try {
            ...
            setContests([...json].filter(contest => contest.status === "CODING"));
            setFilterContests([...json].filter(contest => contest.status === "CODING"))
    ...
    return (
        <div className="container">
            {loading && <Spinner/>}
            {!loading && <TableContainer component={Paper} style={{ width: '100%', boxShadow: darkmode ? '0 0 4px #757575' : '' }}>
                <Table>
                    <TableHead>
                        <TableRow>
